...
Memory::Memory(const std::string_view processName, ACCESS_LEVEL accessLevel) {
    ...
    this->readVirtual = reinterpret_cast<pNtReadVirtualMemory>(GetProcAddress(GetModuleHandleA("ntdll.dll"),
                                                                              "NtReadVirtualMemory"));
    this->writeVirtual = reinterpret_cast<pNtWriteVirtualMemory>(GetProcAddress(GetModuleHandleA("ntdll.dll"),
    ...
    if (!this->attachProcess(accessLevel)) {
        SPDLOG_ERROR("Failed while trying to attach on process {}", processName);

        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Error", "Failed to launch, the process isnt running.", 0);
        exit(EXIT_FAILURE);
    }
...
bool Memory::attachProcess(ACCESS_LEVEL accessLevel) {
    ...
    SPDLOG_INFO("Attached on pid {}, handle {}", this->m_processId, this->m_handle);
